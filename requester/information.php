...
function fetch_current_user_requester(array $user, string $userId, string $userEmail, string $sessionName): array {
  // Prioritize by name field, then id, then email
  try {
    if ($sessionName !== '') {
      // Try to fetch by name first (with requester role filter)
      $rows = supabase_request('GET', 'users', null, ['select' => '*', 'name' => 'eq.' . $sessionName, 'user_type' => 'ilike.requester', 'limit' => 1]);
  ...
  if ($action === 'profile') {
    ...
    if (!$errors) {
      try {
        ...
        elseif ($sessionName !== '') { $query['name'] = 'eq.' . $sessionName; $query['user_type'] = 'ilike.requester'; }
        ...
        if (!is_array($patched) || count($patched) === 0) {
          ...
          if (!isset($row['user_type'])) { $row['user_type'] = 'requester'; }
          if ($userEmail !== '') { $row['email'] = $userEmail; }
          supabase_insert('users', $row);
        }

        foreach (['name','department','birthday','address','signature_image'] as $k) {
...
$dbUser = fetch_current_user_requester($user, $userId, $userEmail, $sessionName);
...
<body>
  ...
  <main class="container">
    <section class="actions" aria-label="Requester actions">
      <a class="btn" href="/ERS/requester/dashboard.php">Back to Home</a>
    </section>
