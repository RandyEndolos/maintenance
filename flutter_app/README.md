Maintenance Flutter + Supabase
=============================

Quick start

- Copy `.env.example` to `.env` and set `SUPABASE_URL` and `SUPABASE_ANON_KEY`.
- Ensure you have a Supabase project and a table named `work_requests`.

Example `work_requests` table SQL (run in Supabase SQL editor):
...
2. Use the app to sign up / sign in (email + password). Create and view work requests.

Notes
- This is a minimal starter. You should configure Row Level Security (RLS) in Supabase tables and policies properly.
- Create a storage bucket (e.g., `public`) and configure permissions if you want attachments to be publicly accessible.
